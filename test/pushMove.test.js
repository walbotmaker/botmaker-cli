...
test('a file dragged into another type folder is told where it belongs, not moved for you', () => {
  const status = {
  ...
  assert.throws(
    ...
    (err) => {
      ...
      assert.match(err.message, /Move it anywhere under 'src\/user\/'/, 'and that any spot under its type works');
      return true;
    }
  );
});
